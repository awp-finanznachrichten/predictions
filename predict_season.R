...
colnames(season_prognosis) <- c("BSC Young Boys","FC Basel 1893","FC Lugano","FC Luzern","FC Sion","FC St. Gallen 1879","FC Thun","FC Zürich","Neuchâtel Xamax FCS","Servette FC")
...
for (a in 1:50) {
  ...
  # Train the model 
  regr <- randomForest(x = X, y = y , maxnodes = 250, ntree = 1100)
  print(regr)
  ...
  prediction_next_game$score_home <- as.numeric(as.character(prediction_next_game$win))*3+as.numeric(as.character(prediction_next_game$draw))
  prediction_next_game$score_away <- as.numeric(as.character(prediction_next_game$loss))*3+as.numeric(as.character(prediction_next_game$draw))
  ...
  #Get scores so far
  current_season <- data_transfermarkt[data_transfermarkt$season == "19/20",]
