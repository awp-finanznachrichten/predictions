...
upcoming_matches$overall_performance_home <- NA
upcoming_matches$overall_performance_away <- NA

old_matches <- matches_database[,c(4:10,14,33:34,45:46,49:52)]
...
for (i in (nrow(all_matches)-length(new_matches)):nrow(all_matches)) {
  ...
  team <- all_matches$team_home[i]
  date <- all_matches$date[i]
  overall <- all_matches[all_matches$team_home == team & all_matches$date < date,]
  performance <- mean(overall$points_home)
  all_matches$overall_performance_home[i] <- performance
...
for (i in (nrow(all_matches)-length(new_matches)):nrow(all_matches)) {
  ...
  team <- all_matches$team_away[i]
  date <- all_matches$date[i]
  overall <- all_matches[all_matches$team_away == team & all_matches$date < date,]
  performance <- mean(overall$points_away)
  all_matches$overall_performance_away[i] <- performance
